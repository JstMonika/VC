...
int main()
{
    ...
    while (cin >> num >> div) {
        ...
        while (true) {
            remainder *= 10;
            rest += to_string(remainder/div);
            remainder %= div;
            
            if (_list.find(remainder) != _list.end()) {
                pos = _list[remainder] + 1;
                break;
            } else {
                _list[remainder] = i++;
            }
        ...
        if (rest.size() - pos > 50) 
            cout << rest.substr(0, pos) << '(' << rest.substr(pos, 50) << "...)" << endl;
        else
            cout << rest.substr(0, pos) << '(' << rest.substr(pos) << ')' << endl;
        cout << "   " << rest.size() - pos << " = number of digits in repeating cycle" << endl;
        cout << endl;
    }
}
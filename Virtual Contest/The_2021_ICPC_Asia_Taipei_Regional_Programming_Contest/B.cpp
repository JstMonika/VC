    ...
    while (c--)
    {
        ...
        for (int i = 0; i < n; i++)
            for (int k = i+1; k < n; k++)
            {
                ...
                for (int m = i; m <= k; m++)
                    if (one[m] == two[k+i-m]) tmp++;
                
                if (tmp + pre[n] - pre[k+1] + pre[i] >= count) {
                    
                    if (k-i < e-f or tmp + pre[n] - pre[k+1] + pre[i] > count) {
                        count = tmp + pre[n] - pre[k+1] + pre[i];
                        f = i;
                        e = k;
                    }
                }

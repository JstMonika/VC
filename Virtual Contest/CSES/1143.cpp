...
struct SegTree {
    ...
    SegTree(int n, vec<int> _list) : n(n), val(_list) {
        st.resize(4*n);
    ...
    int query(int l, int r, int idx, int goal) {
        if (st[idx] < goal)
            return -1;
        ...
        if (st[idx*2] >= goal)
            ret = query(l, m, idx*2, goal);
        else
            ret = query(m+1, r, idx*2+1, goal);
    ...
    /*
    void modify(int l, int r, int idx, int goal, int val) {
        if (l == r) {
            st[idx] -= val;
        }
        ...
        if (goal <= m)
            modify(l, m, idx*2, goal, val);
        else
            modify(m+1, r, idx*2+1, goal, val);

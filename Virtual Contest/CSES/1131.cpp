...
int find_diameter(int now, int parent) {
    
    int ret = 0, one = 0, two = 0;
    for (int i = 0; i < G[now].size(); i++) {
        if (G[now][i] != parent) {
            ret = max(ret, find_diameter(G[now][i], now));
            
            if (height[G[now][i]] > one) {
                two = one;
                one = height[G[now][i]];
            } else if (height[G[now][i]] > two) {
                two = height[G[now][i]];
            }
        }
        ...
        //     cout << ret << endl;
        // if (now == 3) {
        //     cout << G[now][i] << ' ' << one << ' ' << two << endl;
        // }
    }
    
    height[now] = one + 1;
    
    return max(ret, one+two);
}

...
int main()
{
    ...
    while (cin >> n) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            _list[now].resize(k);
            for (int m = 0; m < k; m++)
                cin >> _list[now][m].F >> _list[now][m].S;
        }
        ...
        while (cin >> id) {
            ...
            vec<vec<int>> dis(n+1, vec<int>(n+1));
            for (int i = 0; i < n; i++) {
                int tmp;
                cin >> tmp;
            ...
            ll tans = 0;
            for (int i = 1; i <= n; i++) {
                for (int k = 0; k < _list[i].size(); k++) {
                    tans += _list[i][k].S * dis[i][_list[i][k].F];
                }
            }
        ...
        for (int i = 0; i < ans.size(); i++) {
            cout << setw(5) << ans[i].S << "         " << ans[i].F << endl;
        }
    }
}